...
class _AudioTrimmerViewState extends State<AudioTrimmerView> {
  ...
  void _findTrimmedAudio() async {
    // Get the application documents directory
    final appDocumentsDirectory = await getApplicationDocumentsDirectory();

    // Get the list of files in the trimmed_audio directory
    final trimmedAudioDirectory = Directory('${appDocumentsDirectory.path}/trimmed_audio');
    final files = trimmedAudioDirectory.listSync();

    // Find the latest trimmed audio file based on the creation time
    File? latestTrimmedAudio;
    DateTime latestCreationTime = DateTime(2000); // Some old date
    for (final file in files) {
      final stat = file.statSync();
      if (stat.type == FileSystemEntityType.file && stat.modified.isAfter(latestCreationTime)) {
        latestTrimmedAudio = File(file.path);
        latestCreationTime = stat.modified;
      }
    }

    // Do something with the latest trimmed audio file
    if (latestTrimmedAudio != null) {
      debugPrint('Latest trimmed audio path: ${latestTrimmedAudio.path}');
    } else {
      debugPrint('No trimmed audio files found.');
    }
  }
                ...
                child: Container(
                  ...
                  child: Column(
                    ...
                    children: <Widget>[
                      ...
                      // Add the button to find trimmed audio
                      ElevatedButton(
                        onPressed: _progressVisibility ? null : () => _findTrimmedAudio(),
                        child: const Text("Find Trimmed Audio"),
                      ),
                      Center(
                        child: Padding(
